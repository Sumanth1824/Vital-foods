document.addEventListener('DOMContentLoaded', () => {
    ...
    // Remove references to checkout modal elements as they are no longer needed for direct add to cart
    ...
    let currentProductToAdd = null; // Still good to keep if you want to reuse it for other purposes, but not strictly needed for direct add

    // Store active timeouts for each column
    const activeTimeouts = new Map();

    // Function to clear all active timeouts for a given column
    ...
    // Function to add a timeout to the active timeouts map
    ...
    // Modified addToCart to accept customer details (can be null/empty for direct add)
    function addToCart(product, customerDetails = null) { // customerDetails now has a default null
        const cart = getCart();
        ...
        if (existingProductIndex > -1) {
            ...
            // Optionally update customer details if product already exists
            if (customerDetails) {
                cart[existingProductIndex].customer = customerDetails;
            }
        } else {
            // Add customer details to the product object if provided, otherwise it's null
   ...
   function updateCartCount() {
        ...
        // Optionally hide the count if 0
        if (totalItems === 0) {
    ...
    function createProductColumn(productData) {
        ...
        col.id = `product-column-${id}`; // Assign an ID to each column for identification
        ...
        atc.addEventListener('click', () => {
            ...
            addToCart(productToAdd); // Directly add to cart, customerDetails will be null
            alert(`${productToAdd.name} added to cart!`); // Provide immediate feedback
        });


        vmb.addEventListener('click', () => {
            // Clear all timeouts for all columns before expanding a new one
            document.querySelectorAll('.product-column').forEach(otherCol => {
                clearColumnTimeouts(otherCol.id); // Assuming 'id' is set for each column
            });

            // Show and style the main navigation
            mainNav.classList.remove('hidden');
            mainNav.classList.add('expanded-nav');
            document.getElementById('nav-products-expanded').classList.add('active'); // Activate Products link
            document.getElementById('nav-contact-expanded').classList.remove('active'); // Ensure Contact link is not active
            ...
            document.querySelectorAll('.product-column').forEach(other => {
                ...
                if (other !== col) {
                    ...
                    // Show background icons
                    if (bgIcons) bgIcons.style.display = 'block';

                    // Make the background name visible again
                    ...
                    function typeDescription(callback) {
                        if (col.classList.contains('expanded')) { // Only type if still expanded
                            if (pointIndex < descriptionPoints.length) {
                                ...
                                atc.classList.add('show-btn'); // Ensure button is visible
                                if (callback) {
                                    callback();
                                }
                            }
                    ...
                    typeDescription(typeDosages); // This is the key change: call typeDosages first after description
                }
            });
        });

        sb.addEventListener('click', () => {
            clearColumnTimeouts(id); // Clear timeouts for this specific column when shrinking
            ...
            // Ensure top-right links are visible
            ...
            document.querySelectorAll('.product-column').forEach(c => {
                ...
                c.querySelector('.add-to-cart-btn').classList.remove('show-btn'); // Ensure button is hidden
                ...
                c.querySelectorAll('.quantity-buttons button').forEach(x => {
                    ...
                    if (x.textContent.trim() === '3M') x.classList.add('active'); // Reset to 3M active
                });
    ...
    function renderProducts() {
        ...
        setTimeout(() => {
            // Clear all timeouts for any currently rendered products before re-rendering
    ...
    // Initial calls
    renderProducts();
    updateCartCount(); // Call this on page load to display initial cart count
});
